    ...
    function containsPhrase(row) {
        ...
        for (var i = 0; i < that.columns.length; i++) {
            ...
            if (column.searchable && column.visible) {
                var converted = column.converter.to(row[column.id], column, row);

                if (converted && typeof converted === "string" && converted.search(searchPattern) > -1) {
                    return true;
                }
            }
        }
            ...
            $.each(that.columns, function (j, column) {
                if (column.visible) {
                    var value = ($.isFunction(column.formatter)) ?
                            ...
                            column.converter.to(row[column.id], column, row),
